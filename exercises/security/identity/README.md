...
is refused even though the call only touches orders, because the alternative, reading privileged
and filtering afterwards, is a display preference. The content was still read, still logged,
still put in front of the model, and the backend's own audit log now disagrees with your policy.
