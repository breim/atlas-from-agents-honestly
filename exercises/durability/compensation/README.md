...
response to failure is to finish rather than to reverse, because you cannot un-send an email and
a human has already read it. `the-pivot-decides-the-direction-wherever-it-sits` moves the pivot
...
others, because the remaining holds still need releasing, and the failure has to surface with
a name attached rather than in a log nobody reads.
