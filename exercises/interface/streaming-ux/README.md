...
`connect` is delivered every buffered event past its `lastEventId` — `null` means from the
...
switching tabs, going to lunch, a train entering a tunnel, and changing their mind — and only
