...
three at the node, three at the workflow, each number reasonable and each written by a
different person, is twenty-seven provider calls for one logical step, aimed at a service that
is already struggling. Nested retries multiply. They never add. The fix is not smaller
numbers, it is one owner per class and one attempt everywhere else.
...
nothing like the arithmetic predicted. Bounding it is a context decision, not a retry-policy
one: keep the error out of the transcript.
