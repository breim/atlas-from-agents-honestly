...
mid-arguments on `issue_credit` with `{"cents":90` — which parses as nothing, but a
...
Note that `complete: false` is not an error. It is a fact the caller needs, and what to
do about it — retry, show a partial with an indicator, discard — is their decision, not
the assembler's.
